...
save-alpine-glibc: build-alpine-glibc
	docker save $(PRE)/$(ALPINE_IMG):latest $(PRE)/$(ALPINE_IMG):$(ALPINE_VER) > $(BUILD_PRE)-$(ALPINE_IMG)-$(ALPINE_VER)
	xz --compress --force --threads=0 $(BUILD_PRE)-$(ALPINE_IMG)-$(ALPINE_VER)
...
save-asciidoctor: build-asciidoctor
	docker save $(PRE)/$(ASCIIDOCTOR_IMG):latest $(PRE)/$(ASCIIDOCTOR_IMG):$(ASCIIDOCTOR_VER) > $(BUILD_PRE)-$(ASCIIDOCTOR_IMG)-$(ASCIIDOCTOR_VER)
	xz --compress --force --threads=0 $(BUILD_PRE)-$(ASCIIDOCTOR_IMG)-$(ASCIIDOCTOR_VER)
...
save-openjdk8-jre: build-openjdk8-jre
	docker save $(PRE)/$(OPENJDKJRE_IMG):latest $(PRE)/$(OPENJDKJRE_IMG):$(OPENJDK8JRE_VER) > $(BUILD_PRE)-$(OPENJDKJRE_IMG)-$(OPENJDK8JRE_VER)
	xz --compress --force --threads=0 $(BUILD_PRE)-$(OPENJDKJRE_IMG)-$(OPENJDK8JRE_VER)
...
save-plantuml: build-plantuml
	docker save $(PRE)/$(PLANTUML_IMG):latest $(PRE)/$(PLANTUML_IMG):$(PLANTUML_VER) > $(BUILD_PRE)-$(PLANTUML_IMG)-$(PLANTUML_VER)
	xz --compress --force --threads=0 $(BUILD_PRE)-$(PLANTUML_IMG)-$(PLANTUML_VER)
